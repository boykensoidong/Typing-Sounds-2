...
export function activate(context: vscode.ExtensionContext) {
    console.log('Initializing "typing-sounds" extension');
    ...
    vscode.commands.registerCommand('typing_sounds.enable', () => {
        if (!isActive) {
            ...
            isActive = true;
            vscode.window.showInformationMessage('Typing Sounds extension enabled');
        } else {
            vscode.window.showWarningMessage('Typing Sounds extension is already enabled');
        }
    });
    vscode.commands.registerCommand('typing_sounds.disable', () => {
        if (isActive) {
            ...
            isActive = false;
            vscode.window.showInformationMessage('Typing Sounds extension disabled');
        } else {
            vscode.window.showWarningMessage('Typing Sounds extension is already disabled');
        }
    });
    vscode.commands.registerCommand('typing_sounds.volumeUp', () => {
        ...
        switch (process.platform) {
            case 'darwin':
                ...
                if (config.macVol > 10) {
                    vscode.window.showWarningMessage('Typing Sounds already at maximum volume');
                    config.macVol = 10;
                }
        ...
        vscode.window.showInformationMessage('Typing Sounds volume raised: ' + newVol);
    });
    vscode.commands.registerCommand('typing_sounds.volumeDown', () => {
        ...
        switch (process.platform) {
            case 'darwin':
                ...
                if (config.macVol < 1) {
                    vscode.window.showWarningMessage('Typing Sounds already at minimum volume');
                    config.macVol = 1;
                }
        ...
        vscode.window.showInformationMessage('Typing Sounds volume lowered: ' + newVol);
    });

    vscode.commands.registerCommand('typing_sounds.setVolume', async () => {
